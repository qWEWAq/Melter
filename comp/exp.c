...
static unsigned long Expand(TDcmpStruct * pWork)
{
    ...
    while((dwResult = oneByte = DecodeLit(pWork)) < 0x305)
    {
        ...
        if(pWork->outputPos >= 0x2000)
        {
            ...
            CopyMemory(pWork->out_buff, &pWork->out_buff[0x1000], pWork->outputPos - 0x1000);
        ...
        void         *param)
{
    ...
    if(pWork->ctype != CMP_BINARY)
    {
        ...
        CopyMemory(pWork->ChBitsAsc, ChBitsAsc, sizeof(pWork->ChBitsAsc));
        GenAscTabs(pWork);
    }

    CopyMemory(pWork->LenBits, LenBits, sizeof(pWork->LenBits));
    GenDecodeTabs(0x10, pWork->LenBits, LenCode, pWork->position2);
    CopyMemory(pWork->ExLenBits, ExLenBits, sizeof(pWork->ExLenBits));
    CopyMemory(pWork->LenBase, LenBase, sizeof(pWork->LenBase));
    CopyMemory(pWork->DistBits, DistBits, sizeof(pWork->DistBits));
