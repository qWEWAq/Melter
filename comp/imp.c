...
static void SortBuffer(TCmpStruct * pWork, unsigned char * uncmp_data, unsigned char * work_end)
{
    ...
    if(ndwords <= 1)
        ndwords = 1;
    ZeroMemory(pWork->offs0DC8, ndwords << 2);
...
static void FlushBuf(TCmpStruct * pWork)
{
    ...
    ZeroMemory(pWork->out_buff, 0x802);
...
static void WriteCmpData(TCmpStruct * pWork)
{
    ...
    ZeroMemory(&pWork->out_buff[2], sizeof(pWork->out_buff) - 2);
    pWork->out_bits = 0;
...
loc_15036C7F:
        ...
        uncmp_begin -= 0x1000;
        CopyMemory(pWork->work_buff, &pWork->work_buff[0x1000], UNCMP_OFFSET);
    } while (esp18 == 0);
}
    ...
    unsigned int *dsize)
{
    ...
    CopyMemory(&pWork->dist_codes, DistCode, sizeof(DistCode));
    CopyMemory(&pWork->dist_bits, DistBits, sizeof(DistBits));
    WriteCmpData(pWork);
    return CMP_NO_ERROR;
}
